...
import { alicuotaRouteMap } from '../IataAerRoutesMap'
import { apiAlicuotas } from '../api/alicuotas.api'
import { apiProveedorEliminar } from '../api/proveedorEliminar.api'
import { AlicuotaProps, BoletoIataAereoProps } from '../interfaces/alicuota.interface'
...
import { boaIataAerolineaListado } from '../../ventas/api/boaIataAerolineaListado.api'
import { iataAerolineaEliminar } from '../api/eliminar.api'

interface OwnProps {}

type Props = OwnProps

const tableColumns: MRT_ColumnDef<BoletoIataAereoProps>[] = [
  {
    accessorKey: 'codigoIataLineaAerea',
    header: 'Código Iata Linea Aerea',
    size: 210,
  },
  {
    accessorKey: 'codigoIata',
    header: 'Código Iata',
  },
  {
...
const ProveedorListado: FunctionComponent<Props> = (props) => {
  ...
  // Modifica la función useQuery:
  const { data, isError, isLoading, status, refetch } = useQuery<BoletoIataAereoProps[]>(
    ['codigoIata', columnFilters, pagination.pageIndex, pagination.pageSize, sorting],
    // @ts-ignore
    async () => {
      ...
      const docs = await boaIataAerolineaListado()
  ...
  const handleDeleteData = async (data: any) => {
    const resp = data
      .map((item: any) => Number(item.original.codigoIataLineaAerea))
      .join(',')

    await swalAsyncConfirmDialog({
      text: 'Confirma que desea eliminar los registros seleccionados, esta operación no se podrá revertir',
      preConfirm: () => {
        return iataAerolineaEliminar(resp).catch((err) => {
          swalException(err)
          return false
        })
      },
  ...
  const getCurrentPageItems = () => {
    ...
    // @ts-ignore
    const sortedData = data?.sort((a, b) => b.codigoIata.localeCompare(a.codigoIata))
