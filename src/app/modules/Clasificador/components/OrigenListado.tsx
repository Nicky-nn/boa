...
import { boaOrigenServicioListado } from '../../ventas/api/boaIataAerolineaListado.api'
import OrigenDialog from '../view/OrigenDialog'
import { OrigenServicioEliminar } from '../api/eliminar.api'

interface OwnProps {}

type Props = OwnProps

const tableColumns: MRT_ColumnDef<OrigenProps>[] = [
  {
    accessorKey: 'codigoOrigenServicio',
    header: 'Código Origen Servicio',
    size: 210,
  },
  {
...
const OrigenListado: FunctionComponent<Props> = (props) => {
  ...
  // Modifica la función useQuery:
  const { data, isError, isLoading, status, refetch } = useQuery<OrigenProps[]>(
    [
    ...
    async () => {
      ...
      const docs = await boaOrigenServicioListado()
  ...
  const handleDeleteData = async (data: any) => {
    const resp = data.map((item: any) => item.original.codigoOrigenServicio).join(',')
    await swalAsyncConfirmDialog({
      text: 'Confirma que desea eliminar los registros seleccionados, esta operación no se podra revertir',
      preConfirm: () => {
        return OrigenServicioEliminar(resp).catch((err) => {
          swalException(err)
          return false
        })
      },
  ...
  const getCurrentPageItems = () => {
    ...
    const sortedData = data?.sort((a, b) =>
      b.codigoOrigenServicio.localeCompare(a.codigoOrigenServicio),
    )
      ...
      >
        ...
        >
          <Button
            size={'small'}
            variant="contained"
            onClick={() => setOpenNuevoOrigen(true)}
            startIcon={<Newspaper />}
            color={'success'}
          >
            {' '}
