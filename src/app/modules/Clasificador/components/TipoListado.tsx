...
} from '../../ventas/api/boaIataAerolineaListado.api'
import OrigenDialog from '../view/OrigenDialog'
import TipoDialog from '../view/TipoDialog'
import { TipoTransaccionEliminar } from '../api/eliminar.api'

interface OwnProps {}

type Props = OwnProps

const tableColumns: MRT_ColumnDef<TipoProps>[] = [
  {
    accessorKey: 'codigoTipoTransaccion',
    header: 'Código Tipo Transacción',
    size: 210,
  },
  {
...
const TipoListado: FunctionComponent<Props> = (props) => {
  ...
  // Modifica la función useQuery:
  const { data, isError, isLoading, status, refetch } = useQuery<TipoProps[]>(
    [
    ...
    async () => {
      ...
      const docs = await boaTipoTransaccionListado()
  ...
  const handleDeleteData = async (data: any) => {
    const resp = data.map((item: any) => item.original.codigoTipoTransaccion).join(',')
    await swalAsyncConfirmDialog({
      text: 'Confirma que desea eliminar los registros seleccionados, esta operación no se podra revertir',
      preConfirm: () => {
        return TipoTransaccionEliminar(resp).catch((err) => {
          swalException(err)
          return false
        })
      },
  ...
  const getCurrentPageItems = () => {
    ...
    const sortedData = data?.sort((a, b) =>
      b.codigoTipoTransaccion.localeCompare(a.codigoTipoTransaccion),
    )
